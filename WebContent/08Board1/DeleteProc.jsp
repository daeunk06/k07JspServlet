...
<%
	...
	//String session_id = (String)session.getAttribute("USER_ID");//방법1
	String session_id = session.getAttribute("USER_ID").toString();//방법2
	
	int affected =0;
	//작성자 본인확인을 위해 db에 입력된 작성자아 세션영역에 저장된 속성을 비교한다.
	if(session_id.equals(dto.getId())){
		dto.setNum(num); //dto에 일련번호 저장후
	...
	%>
		<!--
		 <script > 이렇게 쓰면 복잡하니까 위처럼 함수 호출하는거임
		alert("본인만..")
		location.href="";
		history.back();
		</script> -->
