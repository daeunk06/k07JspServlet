...
<body>
	...
	<h3>EL 에서의 null연산</h3>
	<%
	/*Java에선 null과 연산을 수행할 수 없지만 el에선 null을 0으로 간주하여 계산*/
	...
	만약 
		...
		해당페이지?myNumber=      =>0으로 간주 10출력
		해당페이지?myNumber=Three =>문자는 숫자로 변경할 수 없어 에러 -->
	\${param.myNumber + 10 } : ${param.myNum + 10 } <br />
	<br />
	
	<!-- 
		null 값과 비교연산에서는 false를 반환한다
		산술연산에서만 0으로 간주하고 연산을 진행함
	 -->
	...
	<%
	/* 
	El에서는 jsp에서 선언한 변수를 직접 사용할 수 없다 값이 출력되지 않고  null로
	인식하기 때문이다. jsp변수를 el에서 사용할 수 없는 이유는  el은 4가지 영역에 
	...
	*/
	...
	<!-- 
	jsp에서 선언한 변수를 el에서 사용해야할 경우에는 jstl의 set태그를 이용해서 
	변수를 선언한다 jsp에서 선언후 바로 사용하려면 영역에 저장해야한다.
	 -->
