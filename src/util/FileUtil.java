...
public class FileUtil {
	...
	/*
	파일 다운로드 원리 : 원본 파일명과 저장된 파일명을 지정하여 파일 다운로드시 파일명을
	원본 파일명으로 변경한다.
...
try{
	//파일명이 한글일 경우 깨짐현사이 발생하므로 반드시 인코딩해야한다.
	...
	*/
	...
	/*
	 서버에 저장된 파일명을 통해 파일객체를 생성한 후 파일을 다운로드 받을 때 아래
	 설정된 응답헤더를 통해 파일명을 원본파일명으로 변경되어 로컬로
	 다운로드 된다.
	 */
	response.setHeader("Content-Disposition"
